...
func makeReleaseDocs() {
	//branch := flag.String("number-branch", "1-19", "Release branch to test")
	...
	releasePath := fmt.Sprintf("%v/docs/contents/releases/%v/%v", gitRoot, releaseBranch, release)
	print("releasePath: ")
	println(releasePath)
	...
	releaseVersionName := fmt.Sprintf("v%v-eks-%v", releaseBranch, release)
	...
	if err := t.Execute(w, releaseInfo); err != nil {
		panic(err)
	}
	d1 := []byte(release+"\n")
	err = f.Truncate(0)
...
//docs/contents/releases/
	...
	//		}
	//	}
	//	r := regexp.MustCompile("Makefile|cmd/main_postsubmit.go|EKS_DISTRO_BASE_TAG_FILE|number/.*")
